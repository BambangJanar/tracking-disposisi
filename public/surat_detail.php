...
$availableUsers = UsersService::getAll($user['id']); // Exclude current user
...
if (isset($_GET['success'])) {
    switch ($_GET['success']) {
        case 'sent': $successMessage = 'Disposisi berhasil dikirim'; break;
        case 'updated': $successMessage = 'Disposisi berhasil diupdate'; break;
    }
}

if (isset($_GET['error'])) {
    switch ($_GET['error']) {
        case 'failed': $errorMessage = 'Gagal mengirim disposisi'; break;
        case 'invalid': $errorMessage = 'Data tidak valid'; break;
    }
}
?>
...
<div class="flex min-h-screen">
    ...
    <div class="flex-1 lg:ml-64">
        <main class="p-6 lg:p-8">
            ...
            <div class="mb-6 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg relative alert-dismissible" role="alert">
                <div class="flex items-center"><i class="fas fa-check-circle mr-2"></i><span><?= $successMessage ?></span></div>
                <button onclick="this.parentElement.remove()" class="absolute top-0 bottom-0 right-0 px-4 py-3"><i class="fas fa-times"></i></button>
            </div>
            <?php endif; ?>
            
            <?php if ($errorMessage): ?>
            <div class="mb-6 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative alert-dismissible" role="alert">
                <div class="flex items-center"><i class="fas fa-exclamation-circle mr-2"></i><span><?= $errorMessage ?></span></div>
                <button onclick="this.parentElement.remove()" class="absolute top-0 bottom-0 right-0 px-4 py-3"><i class="fas fa-times"></i></button>
            </div>
            <?php endif; ?>
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div><label class="text-sm text-gray-600">Tanggal Diterima</label><p class="font-semibold text-gray-800"><?= formatTanggal($surat['tanggal_diterima']) ?></p></div>
                            <?php endif; ?>
                            ...
                            <div class="md:col-span-2"><label class="text-sm text-gray-600">Ke Instansi</label><p class="font-semibold text-gray-800"><?= $surat['ke_instansi'] ?></p></div>
                            <?php endif; ?>
                            
                            <div class="md:col-span-2"><label class="text-sm text-gray-600">Alamat Surat</label><p class="font-semibold text-gray-800"><?= nl2br($surat['alamat_surat']) ?></p></div>
                            ...
                            <div><label class="text-sm text-gray-600">Dibuat Oleh</label><p class="font-semibold text-gray-800"><?= $surat['dibuat_oleh_nama'] ?></p></div>
                            <div><label class="text-sm text-gray-600">Tanggal Dibuat</label><p class="font-semibold text-gray-800"><?= formatDateTime($surat['created_at']) ?></p></div>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <div class="space-y-3">
                            ...
                            foreach ($disposisiHistory as $disp) { if(isset($statusCount[$disp['status_disposisi']])) $statusCount[$disp['status_disposisi']]++; }
                            ?>
                            
                            <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                                <div class="flex items-center"><i class="fas fa-check-circle text-green-600 mr-2"></i><span class="text-sm text-gray-700">Selesai</span></div>
                                <span class="font-bold text-gray-800"><?= $statusCount['selesai'] ?></span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                                <div class="flex items-center"><i class="fas fa-spinner text-yellow-600 mr-2"></i><span class="text-sm text-gray-700">Diproses</span></div>
...
document.getElementById('disposisiForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    ...
    try {
        ...
        if (!contentType || contentType.indexOf("application/json") === -1) {
            ...
            // Cek jika halaman login atau 401 Unauthorized
            if (response.status === 401 || textResult.includes('Login') || textResult.includes('<!DOCTYPE html>')) {
                throw new Error("Sesi login Anda telah habis. Halaman akan direfresh.");
            } else {
                console.error("Server Response:", textResult);
                throw new Error("Terjadi kesalahan server (Output bukan JSON). Cek Console.");
            }
        }
        ...
        if (result.success) {
            ...
            if (result.html) {
                const timelineContainer = document.getElementById('disposisi-timeline-container');
                if (timelineContainer) timelineContainer.innerHTML = result.html;
            }
            // Update Counter Total
            if (result.count !== undefined) {
                const countElem = document.getElementById('stat-total');
                if (countElem) countElem.innerText = result.count;
            }
        } else {
            throw new Error(result.message || 'Gagal memproses disposisi');
        }
        ...
        Swal.fire({
            ...
            if (error.message.includes('Sesi login')) {
