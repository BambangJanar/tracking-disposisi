...
$query = "SELECT s.*, j.nama_jenis FROM surat s JOIN jenis_surat j ON s.id_jenis = j.id $whereClause ORDER BY s.tanggal_diterima DESC";
$suratList = dbSelect($query, $params, $types);
...
<head>
    ...
    <style>
        ...
        .ttd-wrapper { width: 100%; margin-top: 40px; display: table; }
...
<body>
        ...
        <table>
            <tr>
                ...
                <td class="text-cell">
                    ...
                    <p>
                        <?php if (!empty($settings['instansi_telepon'])): ?>Telp: <?= htmlspecialchars($settings['instansi_telepon']) ?><?php endif; ?>
                        <?php if (!empty($settings['instansi_telepon']) && !empty($settings['instansi_email'])): ?> | <?php endif; ?>
                        <?php if (!empty($settings['instansi_email'])): ?>Email: <?= htmlspecialchars($settings['instansi_email']) ?><?php endif; ?>
                    </p>
                    <?php endif; ?>
                </td>
    ...
    <div class="judul">
        <h3>LAPORAN SURAT DITERIMA (DISETUJUI)</h3>
...
$dompdf->render();
$filename = 'Laporan_Surat_Diterima_' . date('Ymd_His') . '.pdf';
