...
class DisposisiService {
    ...
    // ==========================================
    // UNIQUE INBOX METHODS (DEDUPLICATED)
    // ==========================================

    /**
     * Hitung total surat unik di inbox (untuk pagination)
     * Menghindari double counting jika user mendapat disposisi berkali-kali untuk surat yang sama
     */
    public static function countUniqueInbox($filters = []) {
        $params = [];
        $types = '';

        // Hitung DISTINCT id_surat agar tidak ganda
        $query = "SELECT COUNT(DISTINCT d.id_surat) as total
                  FROM disposisi d
                  JOIN surat s ON d.id_surat = s.id
                  WHERE 1=1";

        // Filter penerima (Wajib ada karena ini Inbox)
    ...
    /**
     * Ambil data inbox unik (1 surat = 1 baris)
     * Mengambil disposisi TERBARU untuk surat tersebut bagi user yang login
     */
    public static function getUniqueInboxByStakeholder($filters = [], $limit = 10, $offset = 0) {
        $params = [];
        $types = '';

        // Kita gunakan Subquery untuk mencari ID disposisi terakhir per surat 
        // khusus untuk user yang sedang login (ke_user_id)
        
        $subQueryWhere = "WHERE ke_user_id = ?";
        $subQueryParams = [$filters['ke_user_id']];
        $subQueryTypes = 'i';

        if (!empty($filters['status_disposisi'])) {
            $subQueryWhere .= " AND status_disposisi = ?";
            $subQueryParams[] = $filters['status_disposisi'];
            $subQueryTypes .= 's';
        }

        // Main Query: Join tabel disposisi dengan hasil MAX(id) dari subquery
        $query = "SELECT d.*, 
        ...
        // Masukkan parameter subquery ke parameter utama
        foreach ($subQueryParams as $p) $params[] = $p;
        $types .= $subQueryTypes;

        // Search Logic (dilakukan di main query karena butuh join tabel surat)
        if (!empty($filters['search'])) {
            $query .= " AND (s.nomor_surat LIKE ? OR s.perihal LIKE ? OR s.nomor_agenda LIKE ?)";
            $searchTerm = '%' . $filters['search'] . '%';
            $params[] = $searchTerm;
