...
class SuratService
{
    ...
    public static function getAll($filters = [], $limit = 10, $offset = 0)
    {
        ...
        if (!empty($filters['tingkat_surat'])) {
            $sql .= " AND s.tingkat_surat = ?";
            $params[] = $filters['tingkat_surat'];
            $types .= "s";
        }
    ...
    public static function updateStatus($id, $status, $alasanPenolakan = null)
    {
        $conn = getConnection();

        // Jika status ditolak dan ada alasan, simpan alasan penolakan
        if ($status === 'ditolak' && $alasanPenolakan) {
            $stmt = $conn->prepare("UPDATE surat SET status_surat = ?, alasan_penolakan = ? WHERE id = ?");
